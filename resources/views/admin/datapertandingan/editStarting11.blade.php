...
<div class="container mt-4">
    <h2 class="mb-4 text-center">Edit Starting 11 dan Posisi</h2>
    ...
    <form action="{{ route('datapertandingan.updateStarting11', $datapertandingan->id) }}" method="POST" id="starting11Form">
        ...
        <div class="row mb-3">
            ...
            <div class="col-md-6 mb-2">
                <select class="form-select player-select" name="players[{{ $i }}][player_name]" required>
            ...
            <!-- Position Selection -->
            <div class="col-md-6 mb-2">
                <select class="form-select position-select" name="players[{{ $i }}][position]" required>
    ...
    <!-- FIELD: Drag & Drop for Player Positions -->
    <h4 class="mt-4 text-center text-success">Atur Posisi Pemain di Lapangan</h4>
<div id="field" 
    style="position: relative; width: 100%; height: 500px; background: url('{{ asset('img/field.jpg') }}') no-repeat center center; background-size: cover; border: 2px solid black;">
    ...
    <!-- FORM: Save Positions -->
    <form action="{{ route('datapertandingan.updatePositions', $datapertandingan->id) }}" method="POST" id="positionForm">
        @csrf
        @method('PUT')
...
<!-- JavaScript for Preventing Duplicate Selection -->
<script>
    ...
    function updateAvailableOptions() {
        ...
        playerSelects.forEach(select => {
            ...
            options.forEach(option => {
                if (option.value && selectedPlayers.has(option.value) && option.value !== select.value) {
